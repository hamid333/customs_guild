...
/** مقداردهی اولیه‌ی CKEditor روی تمام <textarea class="django_ckeditor_widget"> داخل یک بازه.
 * نمونه‌ی قبلی همان فیلد (در صورت وجود) قبل از ساخت مجدد نابود می‌شود تا خطای «قبلاً
 * مقداردهی شده» رخ ندهد (مثلاً وقتی مودال دوباره باز می‌شود). */
function initCKEditor(scope) {
  if (typeof CKEDITOR === "undefined") return;
  const root = scope && scope.querySelectorAll ? scope : document;
  root.querySelectorAll("textarea.django_ckeditor_widget").forEach(function (ta) {
    if (!ta.id) return;
    if (CKEDITOR.instances[ta.id]) {
      try { CKEDITOR.instances[ta.id].destroy(true); } catch (e) { /* noop */ }
    }
    let config = {};
    try {
      if (ta.dataset && ta.dataset.config) config = JSON.parse(ta.dataset.config);
    } catch (e) { /* noop */ }
    CKEDITOR.replace(ta.id, config);
  });
}

/** نابودی نمونه‌های CKEditor داخل یک بازه (پیش از حذف آن بازه از DOM، مثلاً هنگام بسته‌شدن مودال). */
function destroyCKEditorsIn(scope) {
  if (typeof CKEDITOR === "undefined" || !scope || !scope.querySelectorAll) return;
  scope.querySelectorAll("textarea.django_ckeditor_widget").forEach(function (ta) {
...
/** پیش از خواندن FormData از فرم، محتوای هر CKEditor داخل آن فرم را در <textarea> اصلی‌اش
 * همگام می‌کند (چون ارسال ما با fetch است، نه submit واقعی مرورگر که CKEditor خودش گوش می‌دهد). */
function syncCKEditorsToTextareas(form) {
  if (typeof CKEDITOR === "undefined") return;
  ...
  fetch(url, { headers: { "X-Requested-With": "XMLHttpRequest" } })
    ...
    .then(function (html) {
      Swal.fire({
        ...
        willClose: function () {
          destroyCKEditorsIn(Swal.getHtmlContainer());
        },
        preConfirm: function () {
          const form = document.getElementById("modalForm");
          syncCKEditorsToTextareas(form);
